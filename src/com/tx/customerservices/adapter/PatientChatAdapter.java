...
public class PatientChatAdapter extends SimpleCursorAdapter {

	private static final int DELAY_NEWMSG = 2000;
	private Context mContext;
	private LayoutInflater mInflater;

	public PatientChatAdapter(Context context, Cursor cursor, String[] from) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		String message =cursor.getString(cursor
				.getColumnIndex(ChatProvider.ChatConstants.WHAT_NAME))+ (kuaizheng_id != 0 ? "助手":"患者：") +cursor.getString(cursor
		...
		if (convertView == null
			...
			if (come == ChatConstants.OUTGOING) {
				convertView = mInflater.inflate(R.layout.patient_chat_item_right,
						parent, false);
			} else {
				convertView = mInflater.inflate(R.layout.patient_chat_item_left, null);
			}
	...
	private void markAsRead(int id) {
		Uri rowuri = Uri.parse("content://" + PatientChatProvider.AUTHORITY + "/"
				+ PatientChatProvider.TABLE_NAME + "/" + id);
		L.e("markAsRead: " + rowuri);
		ContentValues values = new ContentValues();
		values.put(PatientChatConstants.DELIVERY_STATUS, PatientChatConstants.DS_SENT_OR_READ);
